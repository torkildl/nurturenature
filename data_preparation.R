...
if (STEP1==TRUE) {
  ...
  pcabatch <- fread(here("../../../data/genetics/MoBaPsychGen_v1/MoBaPsychGen_v1-ec-eur-batch-basic-qc-cov-noMoBaIDs.txt")) %>% 
    select(IID, FID, BATCH = genotyping_batch, num_range("PC",1:10))
  # Add scores to the mix
  all_families_with_pgis <- all_families %>% 
    left_join(prefixit(pcabatch, prefix="child_"), by=c(child_SENTRIX_ID = "child_IID")) %>% 
    left_join(prefixit(pcabatch, prefix="mother_"), by=c(mother_SENTRIX_ID = "mother_IID")) %>% 
    left_join(prefixit(pcabatch, prefix="father_"), by=c(father_SENTRIX_ID = "father_IID")) %>% 
  ...
  fwrite(complete_trios, here("noncognitive-cognitive/revision/data/complete-trios.csv"))
  message("Step 1. General trio data set created")
} else {
  complete_trios <- fread(here("noncognitive-cognitive/revision/data/complete-trios.csv"))
...
if (STEP2==TRUE) {
  ...
  sibtrios <- complete_trios %>% 
    select(contains("w19_0634_lnr"), one_of(outcomes), 
           ends_with("_EA4"),ends_with("_Cog"),
           ends_with("_NonCog"), ends_with("WFEA"),
  ...
  fwrite(sibtrios, here("noncognitive-cognitive/revision/data/sibtrios.csv"))
  message("Step 2. Parent-sib data set created")
} else {
  sibtrios <- fread(here("noncognitive-cognitive/revision/data/sibtrios.csv"))
